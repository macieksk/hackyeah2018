## Maciek Sykulski ~2012-2018 macieksk@gmail.com
library("hash")
library("fastmatch")
...
## set options width and height of a canvas to plot on
psz<-function(w,h=w,...)options(repr.plot.width=w, repr.plot.height=h,...)

## Execute function with confirmation
safe_command_run <- function(cmd, ...){
    x <- readline(paste("Do you want to run: ", substitute(cmd), " function? [Y/N] ", sep=""))
    if(x == "Y" | x =='y'){ system.time(cmd(...)); print(paste(substitute(cmd), "done.")) }
    else { print(paste(substitute(cmd), "canceled.")) }
}
